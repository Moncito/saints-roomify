...
const PUBLIC_PREFIX  = 'roomify_feed_';
...
// Trusted hosting domain
const HOSTING_DOMAIN_SUFFIX = '.puter.site';

// Check if a string is a hosted URL on our trusted domain (not base64)
...
router.post('/api/projects/save', async ({ request, user }) => {
    try {
        ...
        if (shouldPublish) {
            ...
            const publicKey = `${PUBLIC_PREFIX}${fKey}`;
...
router.get('/api/projects/feed', async ({ request, user }) => {
    try {
        ...
        const entries = await puter.kv.list(PUBLIC_PREFIX, true);
...
router.post('/api/projects/upvote', async ({ request, user }) => {
    try {
        ...
        await puter.kv.set(publicKey, { ...project, upvotes });
